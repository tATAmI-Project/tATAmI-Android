 ...
 * <p>
 * This implementation presumes that {@link Timer} and related classes {@link TimerTask} are available on the
 * execution platform.
 * 
 * @author Andrei Olaru
 */
public class SimulationManager implements AgentManager
{
	...
	protected boolean setupGui()
	{
		gui.connectInput(SimulationComponent.EXIT.toString(), new AgentActiveIO.InputListener() {
			...
			{
				gui.background(new AgentGui.AgentGuiBackgroundTask() {
					@Override
					public void execute(Object arg, AgentGui.ResultNotificationListener resultListener)
					{
						stop();
						resultListener.receiveResult(null);
					}
				}, null, new AgentGui.ResultNotificationListener() {
					@Override
		...
		{
			gui.connectInput(SimulationComponent.CREATE.toString(), new AgentActiveIO.InputListener() {
				@Override
				public void receiveInput(String componentName, Vector<Object> arguments)
				{
					createAgents();
			...
			gui.connectInput(SimulationComponent.CLEAR.toString(), new AgentActiveIO.InputListener() {
				@Override
				public void receiveInput(String componentName, Vector<Object> arguments)
				{
					signalAllAgents(AgentEvent.AgentEventType.AGENT_STOP);
				}
			});
			
		}
		
		gui.connectInput(SimulationComponent.START.toString(), new AgentActiveIO.InputListener() {
			@Override
			public void receiveInput(String componentName, Vector<Object> arguments)
			{
				gui.background(new AgentGui.AgentGuiBackgroundTask() {
		...
		gui.connectInput(SimulationComponent.PAUSE.toString(), new AgentActiveIO.InputListener() {
			@Override
			public void receiveInput(String componentName, Vector<Object> arguments)
			{
				if(events.isEmpty())
	...
	protected boolean startSimulationAgents()
	{
		for(PlatformLoader platform : platforms.values())
		{
			...
			try
			{
				String msgrClass = platform.getRecommendedComponentClass(AgentComponent.AgentComponentName.MESSAGING_COMPONENT);
				if(msgrClass == null)
					msgrClass = AgentComponent.AgentComponentName.MESSAGING_COMPONENT.getClassName();
