...
public class BootSettingsManager extends Config
{
	...
	public XMLTree load(String programArguments[], boolean parseScenarioFile) throws ConfigLockedException
	{
		...
		case 5:
			...
			{
				Log.e("BootSettingsManager", "unable to parse value for local port; value not retained.");
			}
			//$FALL-THROUGH$
		case 4:
		...
		case 3:
			...
			{
				Log.e("BootSettingsManager", "unable to parse value for main port; value not retained.");
			}
			//$FALL-THROUGH$
		case 2:
		...
		case 1:
			if(!"default".equals(programArguments[0]) && new File(programArguments[0]).exists())
				...
				Log.e("BootSettingsManager", "file [" + programArguments[0] + "] not found.");
			//$FALL-THROUGH$
		case 0:
		}
		...
		{
			Log.i("BootSettingsManager", "loading scenario [" + scenarioFileName + "]");

            InputStream scenarioInputStream = null;
            InputStream schemaInputStream = null;

            try{
                scenarioInputStream =  MainActivity.context.getAssets().open("src-scenario/scenario/dev/mobility/scenario-mobility.xml");
                schemaInputStream = MainActivity.context.getAssets().open("src-schema/scenarioSchema3.xsd");
            }
            catch(Exception e){
                Log.v("BootSettingsManager", "Error occured");
            }


			scenarioTree = XMLParser.validateParse(schemaInputStream, scenarioInputStream);
			
			if(scenarioTree == null)
			{
				Log.e("BootSettingsManager", "scenario parsing result is null.");
				return scenarioTree;
			}
			
			Log.i("BootSettingsManager", "scenario:");
			Log.i("BootSettingsManager", scenarioTree.toString());
        ...
        Log.i("BootSettingsManager", "network config: Main:[]:[] Local:[]:[]" + mainHost == null ? "<null>" : mainHost +
